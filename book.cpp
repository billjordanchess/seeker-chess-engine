...
int book_board[64], book_color[64];
...
int Book()
{
	...
	for (int x = 0; x < total2; x++)
	{
		...
		{
			from = lines[x][y] - 'a';
			from += ((lines[x][y + 1] - '0') - 1) * 8;
			to = lines[x][y + 2] - 'a';
			to += ((lines[x][y + 3] - '0') - 1) * 8;
			book_board[to] = book_board[from];
			book_color[to] = book_color[from];
			book_board[from] = EMPTY;
			book_color[from] = EMPTY;
		}
		match = 1;
		for (int z = 0; z < 64; z++)
		{
			...
			if (mask[z] & bit_units[1])
				colour = 1;
			if ((b[z] != book_board[z]) || (colour != book_color[z]))
			{
				match = 0; break;
			}
		}
...
void InitBookboard()
{
	for (int x = 0; x < 64; x++)
	{
		book_board[x] = start_board[x];
		book_color[x] = start_color[x];
	}
}
